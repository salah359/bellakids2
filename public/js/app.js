...
// UPDATED: Filter Function (Case Insensitive for Age)
function filterGrid() {
    const grid = document.getElementById('product-grid');
    if (!grid) return;
    ...
    const filtered = state.products.filter(p => {
        ...
        // 3. Age Filter (Case Insensitive: "0-3m" matches "0-3M")
        let matchesAge = true;
        if (age !== 'all') {
...
// Modal with Thumbnails and Quantity
function openProductModal(id) {
    ...
    if (thumbContainer) {
        if (images.length > 1) {
            ...
            thumbContainer.innerHTML = images.map((img, idx) => `
                <img src="${resolveImage(img)}" 
                     class="thumb-img rounded shadow-sm" 
                     width="50" height="50" 
                     style="object-fit: cover;"
                     onclick="goToSlide(${idx}, this)">
            `).join('');
        } else {
            thumbContainer.parentElement.classList.add('d-none');
        }
    }
    ...
    btn.onclick = () => {
        ...
        const activeIndex = document.querySelector('#productCarousel .carousel-item.active img').src;
        addToCart(p, state.selectedSize, activeIndex, state.modalQty);
...
function updateModalQtyUI() {
    ...
    if(input) input.value = state.modalQty;
}

function addToCart(p, size, imgUrl, qty) {
    const name = state.lang === 'ar' ? (p.name_ar || p.name_en) : (p.name_en || p.name_ar);
    const exist = state.cart.find(x => x.id === p._id && x.size === size);
    if(exist) {
        exist.qty += qty; 
    } else {
        state.cart.push({ id: p._id, itemId: p.itemId, name, price: p.price, img: imgUrl, size, qty: qty });
    }
    saveCart();
}
        ...
        div.innerHTML += `
            <div class="d-flex align-items-center mb-3">
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="text-muted small">${t.size}: ${x.size} | ${t.currency}${x.price} x ${x.qty}</div>
                </div>
