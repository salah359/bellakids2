...
const ProductSchema = new mongoose.Schema({
    ...
    isSeason: { type: Boolean, default: false }, // <--- ADD THIS LINE HERE
...
app.post('/api/products', requireAuth, upload.array('images', 10), async (req, res) => {
    try {
        const { itemId, name_en, name_ar, price, oldPrice, category, subCategory, sizes, outOfStockSizes, colors, description_en, description_ar, inStock, isEid, isSeason, imageCodes } = req.body;
        ...
        const newProduct = new Product({
            ...
            isSeason: isSeason === 'true' || isSeason === true
        });
...
const { itemId, name_en, name_ar, price, oldPrice, category, subCategory, sizes, outOfStockSizes, colors, description_en, description_ar, inStock, isEid, isSeason, imageCodes } = req.body;
        ...
        product.isSeason = isSeason === 'true' || isSeason === true;
...
// Quick toggle for Season Status
app.put('/api/products/:id/toggle-season', requireAuth, async (req, res) => {
    try {
        const product = await Product.findById(req.params.id);
        if (product) {
            product.isSeason = !product.isSeason; 
            await product.save();
            res.json(product);
        } else {
            res.status(404).json({ error: "Not found" });
...
app.get('/season', (req, res) => res.sendFile(path.join(__dirname, 'public', 'season.html')));
