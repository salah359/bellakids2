...
// --- MODEL ---
// UPDATED: Schema now supports image objects with IDs and subCategory
const ProductSchema = new mongoose.Schema({
    ...
    subCategory: { type: String, default: 'other' }, // NEW FIELD
    sizes: [String],
    colors: [String],
...
app.post('/api/products', requireAuth, upload.array('images', 10), async (req, res) => {
    try {
        const { itemId, name_en, name_ar, price, oldPrice, category, subCategory, sizes, colors, description_en, description_ar, inStock } = req.body;
...
app.put('/api/products/:id', requireAuth, upload.array('images', 10), async (req, res) => {
    try {
        ...
        if(subCategory) product.subCategory = subCategory; // NEW
